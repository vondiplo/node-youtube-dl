...
ytdl.setupExePath = function(path){
      fs.exists(path, function(exists) {
          if (exists) {
            ytdlBinary = path
          }else{
            console.log('couldn\'t locate youtubedl path, falling back on default location');
          }
      });
}

ytdl.hasSub = function(url, cookies, callback) {
    var args = ['--skip-download', '--list-subs'];
    if (cookies)
        args.push('--cookies=' + cookies);
  call(url, args, null, { }, function(err, data) {
      if (err) return callback(err, false);
      var hasSub = false;
      for (var i = 0, len = data.length; i < len && !hasSub; i++) {
          var line = data[i].toLowerCase();
          hasSub = line.indexOf('available subtitles') != -1 ||
                   line.indexOf('available automatic captions') != -1;
      }
      callback(err, hasSub);
  });
}
 ...
 * @param {Function(!Error, Object)} callback
 */
ytdl.getSubs = function(url, args, cwd, callback) {
  if (typeof options === 'function') {
    callback = options;
    options = {};
  }
  var options = cwd ? {cwd: cwd} : {};
  call(url, args, null, options, function(err, data) {
    if (err) return callback(err);
